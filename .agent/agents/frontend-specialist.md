...
model: inherit
skills: clean-code, react-best-practices, web-design-guidelines, tailwind-patterns, frontend-design, lint-and-validate, frappe-development
---
...
### Frappe Desk & Healthcare UI

- **Frappe Desk**: Standard views — List, Form, Report, Dashboard, Workspace; extend via Client Scripts (JS)
- **Frappe UI (Vue 3)**: For custom pages/portals beyond Desk — uses Frappe UI component library
- **Client Scripts**: JavaScript hooks on forms — `frm.trigger`, `frm.set_query`, `frm.set_value` for dynamic behavior
- **Healthcare Patterns**: Patient dashboards (vitals timeline, encounter history), clinical forms (structured encounter notes), appointment calendars
- **Accessibility**: WCAG 2.1 AA mandatory for clinical apps — high contrast, large touch targets, keyboard-first data entry
