...
model: inherit
skills: clean-code, database-design, frappe-development
---
...
- Healthcare/EHR (Frappe)? → MariaDB (default) or PostgreSQL, DocType-driven modeling
...
| Healthcare/EHR (Frappe) | MariaDB + Frappe DocTypes |
...
| Frappe/ERPNext apps | Frappe DocTypes (no ORM needed) |
...
### Frappe DocType Modeling (Healthcare / EHR)
- **DocTypes**: Metadata-driven tables — define fields, permissions, workflows in JSON
- **Link Fields**: Relationships via `Link` type (not raw FKs); Frappe enforces integrity
- **Child Tables**: One-to-many via `Table` field type (e.g., Prescription items inside Encounter)
- **Naming Rules**: `autoname` options — hash, field-based, format strings, Naming Series
- **Migrations**: Automatic via `bench migrate`; schema changes driven by DocType JSON
- **Virtual Fields**: Computed fields via controller `validate` or `before_save`
- **Healthcare DocTypes**: Patient, Patient Appointment, Patient Encounter, Vital Signs, Lab Test, Prescription
- **Best Practice**: Never use raw SQL for CRUD — use `frappe.get_doc`, `frappe.db.get_value`, `frappe.db.set_value`
